...
export class ReportComponent implements OnInit {
  ...
  get submittedTasks() { return this.tasks.filter(t => t.status === 'SUBMITTED' || t.status === 'APPROVED'); }
  get approvedCount() { return this.tasks.filter(t => t.status === 'APPROVED').length; }
  get submittedCount() { return this.tasks.filter(t => t.status === 'SUBMITTED').length; }
